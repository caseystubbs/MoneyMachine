...
export async function POST() {
  ...
  try {
    ...
    // PayPal only allows 31-day windows — chunk the range
    const chunks: { start: Date; end: Date }[] = [];
    let cs = new Date(startDate);
    while (cs < endDate) {
      const ce = new Date(cs);
      ce.setDate(ce.getDate() + 30);
      if (ce > endDate) ce.setTime(endDate.getTime());
      chunks.push({ start: new Date(cs), end: new Date(ce) });
      cs = new Date(ce);
      cs.setSeconds(cs.getSeconds() + 1);
    }

    for (const chunk of chunks) {
      const startStr = chunk.start.toISOString().split(".")[0] + "Z";
      const endStr = chunk.end.toISOString().split(".")[0] + "Z";
      let page = 1;
      let totalPages = 1;
    ...
    return NextResponse.json({ success: true, source: "paypal", txAdded, txUpdated, duration: `${duration}ms` });
  } catch (error) {
    const duration = Date.now() - startTime;
